...
model = dict(
    ...
    backbone=dict(
        ...
        in_channels=3,
        num_stages=5,
        num_layers=[1, 1, 1, 1, 1],  # num of transformer
        patch_sizes=[4, 4, 4, 4, 4],  # patch_embedding size[3, 3, 3, 3, 3]
        strides=[2, 2, 2, 2, 2],
        paddings=[1, 1, 1, 1, 1],
        num_heads=[1, 2, 5, 8, 16],
        swin_num_heads=[1, 2, 5, 8, 16],  # num of transformer_head
        # 网格
        chratio=[16, 8, 4, 2, 1],
        mlp_ratios=[8, 8, 4, 4, 4],
        # The ratio of the mlp hidden dim to the embedding dim of each transformer encode layer
        relative_coords_table_type='norm8_log_bylayer',
        out_indices=(0, 1, 2, 3, 4),
        window_size=[16, 16, 16, 8, 8],
        pretrain_window_size=[12, 12, 12, 6, 6],
        use_shift=[True, True, False, False, False],
        checkpoint_blocks=[255, 255, 255, 255, 255],
        mlpfp32_layer_blocks=[[-1], [-1], [-1], [-1], [-1]],
        ),
        neck=dict(
...
samples_per_gpu_new=2 #Sample 
workers_per_gpu_new=2 #worker  
